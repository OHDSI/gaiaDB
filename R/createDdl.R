...
createDdl <- function(cdmVersion){
  ...
  # argument checks
  stopifnot(is.character(cdmVersion), length(cdmVersion) == 1, cdmVersion %in% c("0.1"))

  cdmTableCsvLoc <- "./csv/gaia001tableLevel.csv"
  cdmFieldCsvLoc <- "./csv/gaia001fieldLevel.csv"
  ...
  sql_result <- c()
  sql_result <- c(paste0("--@targetDialect DDL Specification for Gaia Data Model ", cdmVersion))
